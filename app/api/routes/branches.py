...
from app.schemas import BranchCreateRequest, BranchResponse, ResponseMessage
...
async def create_branch(branch: BranchCreateRequest, _user: str = Depends(get_current_user)):
    row = await create_branch_db(group_id=branch.group_id, name=branch.name, alert=branch.alert)
    if not row:
