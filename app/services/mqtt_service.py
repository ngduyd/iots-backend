...
def on_connect(client, userdata, flags, rc):
    if rc == 0:
        ...
        print(f"[MQTT-SERVICE] Connection failed with code {rc}")
...
def on_message(client, userdata, msg):
    ...
    # Topic format: group/branch/sensors/sensor_id
    ...
    if payload.strip().lower() == "offline":
        if sensor_id and loop:
            asyncio.run_coroutine_threadsafe(sensor_service.update_sensor_status(sensor_id, "offline"), loop)
        return
